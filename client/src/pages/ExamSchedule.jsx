  ...
  return <div className="page-shell">
    ...
    <section className="panel">
      ...
      {loading ? <div className="tool-empty">Loading exam schedule…</div> :
        rows.length ? <div className="leave-table-wrap"><table className="modern-table"><thead><tr><th>Subject</th><th>Exam</th><th>Date</th><th>Time</th><th>Instructions</th></tr></thead><tbody>
          {rows.map(x=><tr key={x.id}><td><strong>{x.subject_name}</strong><br/><span>{x.subject_code}</span></td><td>{x.exam_type}</td><td>{new Date(`${x.exam_date}T00:00:00`).toLocaleDateString()}</td><td>{x.start_time?.slice(0,5)}{x.end_time ? ` – ${x.end_time.slice(0,5)}` : ''}</td><td>{x.instructions || '—'}</td></tr>)}
