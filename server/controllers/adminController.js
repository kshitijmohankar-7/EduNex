...
async function createSemester(req,res,next){try{const {departmentId,courseId,number}=req.body;if(!departmentId||!courseId||!number)return res.status(400).json({error:'Department, course and semester number are required.'});const check=await pool.query('SELECT id FROM courses WHERE id=$1 AND department_id=$2',[courseId,departmentId]);if(!check.rows.length)return res.status(400).json({error:'Selected course does not belong to the selected department.'});const r=await pool.query('INSERT INTO semesters(course_id,number) VALUES($1,$2) RETURNING *',[courseId,number]);res.status(201).json(r.rows[0]);}catch(err){if(err.code==='23505')return res.status(409).json({error:'That semester already exists for this course.'});next(err);}}
async function updateSemester(req,res,next){try{const check=await pool.query('SELECT id FROM courses WHERE id=$1 AND department_id=$2',[req.body.courseId,req.body.departmentId]);if(!check.rows.length)return res.status(400).json({error:'Selected course does not belong to the selected department.'});const r=await pool.query('UPDATE semesters SET course_id=$1,number=$2 WHERE id=$3 RETURNING *',[req.body.courseId,req.body.number,Number(req.params.id)]);if(!r.rows.length)return res.status(404).json({error:'Semester not found.'});res.json(r.rows[0]);}catch(err){next(err);}}
...
async function listDivisions(req,res,next){try{const r=await pool.query(`SELECT v.id,v.name,v.semester_id,d.id AS department_id,c.id AS course_id,sem.number AS semester_number,c.name AS course,d.name AS department FROM divisions v LEFT JOIN semesters sem ON sem.id=v.semester_id LEFT JOIN courses c ON c.id=sem.course_id LEFT JOIN departments d ON d.id=c.department_id ORDER BY d.name,c.name,sem.number,v.name`);res.json(r.rows);}catch(err){next(err);}}
async function createDivision(req,res,next){try{const {departmentId,courseId,semesterId,name}=req.body;if(!departmentId||!courseId||!semesterId||!name)return res.status(400).json({error:'Department, course, semester and section name are required.'});const check=await pool.query('SELECT sem.id FROM semesters sem JOIN courses c ON c.id=sem.course_id WHERE sem.id=$1 AND c.id=$2 AND c.department_id=$3',[semesterId,courseId,departmentId]);if(!check.rows.length)return res.status(400).json({error:'Selected semester does not belong to the selected department and course.'});const r=await pool.query('INSERT INTO divisions(semester_id,name) VALUES($1,$2) RETURNING *',[semesterId,String(name).trim()]);res.status(201).json(r.rows[0]);}catch(err){next(err);}}
async function updateDivision(req,res,next){try{const check=await pool.query('SELECT sem.id FROM semesters sem JOIN courses c ON c.id=sem.course_id WHERE sem.id=$1 AND c.id=$2 AND c.department_id=$3',[req.body.semesterId,req.body.courseId,req.body.departmentId]);if(!check.rows.length)return res.status(400).json({error:'Selected semester does not belong to the selected department and course.'});const r=await pool.query('UPDATE divisions SET semester_id=$1,name=$2 WHERE id=$3 RETURNING *',[req.body.semesterId,String(req.body.name||'').trim(),Number(req.params.id)]);if(!r.rows.length)return res.status(404).json({error:'Division not found.'});res.json(r.rows[0]);}catch(err){next(err);}}
