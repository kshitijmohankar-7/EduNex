...
async function syncNotifications(userId){
  ...
  if(user.role==='faculty'){
    const submissions=await pool.query(`SELECT ass.id,ass.submitted_at,a.title,u.full_name AS student_name FROM assignment_submissions ass JOIN assignments a ON a.id=ass.assignment_id JOIN students st ON st.id=ass.student_id JOIN users u ON u.id=st.user_id WHERE a.uploaded_by=$1 ORDER BY ass.submitted_at DESC LIMIT 30`,[user.faculty_id]);
    for(const s of submissions.rows)rows.push({type:'submission',title:`Assignment submitted: ${s.title}`,message:`${s.student_name||'A student'} submitted an assignment for your review.`,link:'/faculty/assignments',sourceType:'submission',sourceId:s.id,createdAt:s.submitted_at});
  }
  for(const n of rows){await pool.query(`INSERT INTO notifications(user_id,type,title,message,link,source_type,source_id,created_at) VALUES($1,$2,$3,$4,$5,$6,$7,$8) ON CONFLICT (user_id,source_type,source_id) DO NOTHING`,[userId,n.type,n.title,n.message,n.link,n.sourceType,n.sourceId,n.createdAt||new Date()])}
}
