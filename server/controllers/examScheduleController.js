  ...
  try {
    const result = await pool.query(`
      SELECT e.id, e.subject_id, s.name AS subject_name, s.code AS subject_code,
             e.exam_type, e.exam_date, e.start_time, e.end_time,
...
function validatePayload(body) {
  ...
  return { value: { subjectId, examType: examType || 'END SEMESTER', examDate, startTime, endTime: endTime || null, instructions: instructions || null } };
}
...
async function createExam(req, res, next) {
  try {
    ...
    const result = await pool.query(
      `INSERT INTO exam_schedules(subject_id,exam_type,exam_date,start_time,end_time,instructions,created_by)
       VALUES($1,$2,$3,$4,$5,$6,$7)
       RETURNING *`,
      [v.subjectId,v.examType,v.examDate,v.startTime,v.endTime,v.instructions,req.user.id]
    );
